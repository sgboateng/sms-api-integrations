...
def main():
    ...
    # Invoke send SMS function - Npontu API
    sms_sender_npontu_api(df_contacts)
...
# Npontu API
def sms_sender_npontu_api(contacts: pd.DataFrame) -> None:

    # API Credentials - LIVE
    npontu_user_name = "Zenith Bank"
    npontu_passsword = "4#zfVo#*1#z4M>!Tq4P"
    npontu_api_url = "https://deywuro.com/api/sms"
    ...
    for irow in contacts.itertuples(index=True, name='Pandas'):
        ...
        # SMS PayLoad
        payload = {
            "username": npontu_user_name,
            "password": npontu_passsword,
            "source": "ZENITHBANK",
        ...
        # Send the request with TLS 1.2+ and certificate validation
        try:
            response = requests.post(npontu_api_url, data=payload, headers=headers)
